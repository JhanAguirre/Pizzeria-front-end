<template>
  <div class="container text-start mt-4">
    <h1 class="text-primary fw-bold">Editar Orden de Pizza</h1>
    <div class="card shadow-sm">
      ...
      <div class="card-body">
        ...
        <form v-else @submit.prevent="updateOrderPizza">
          <div class="row mb-3">
...
export default {
  ...
  methods: {
    ...
    async fetchOrderAndRelatedData() {
      this.loading = true;
      this.error = null;
      try {
        const orderId = this.$route.params.id;
        const [orderRes, pizzasRes, ingredientsRes] = await Promise.all([
          axios.get(`http://127.0.0.1:8000/api/order_pizzas/${orderId}`),
          axios.get('http://127.0.0.1:8000/api/pizzas'),
          axios.get('http://127.0.0.1:8000/api/extra_ingredients')
        ]);

        this.orderPizza = orderRes.data.orderPizza;
        this.pizzas = pizzasRes.data.pizzas;
        this.extraIngredients = ingredientsRes.data.extraIngredients;

        // Pre-seleccionar pizzas e ingredientes extras de la orden existente
        this.selectedPizzas = this.orderPizza.pizzas.map(pizza => pizza.id);
        this.selectedExtraIngredients = this.orderPizza.extra_ingredients.map(ingredient => ingredient.id);

        this.calculateTotalPrice(); // Calcular el total inicial
      } catch (error) {
        this.error = 'No se pudo cargar la información de la orden o sus datos relacionados.';
        console.error('Error fetching data for edit order:', error);
    ...
    async updateOrderPizza() {
      ...
      this.error = null;
      try {
        const res = await axios.put(`http://127.0.0.1:8000/api/order_pizzas/${this.orderPizza.id}`, this.orderPizza);
        if (res.status === 200) {
          this.$router.push({ name: 'OrderPizzas' });
          Swal.fire({
            position: 'top-end',
            icon: 'success',
            title: '¡Orden de pizza actualizada correctamente!',
        ...
        this.error = 'No se pudo actualizar la orden de pizza.';
        if (error.response) {
        ...
        Swal.fire('Error', this.error, 'error');
        console.error('Error updating order pizza:', error);
      } finally {
        this.loading = false;
      }
    }
